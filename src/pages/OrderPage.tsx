...
const OrderPage = () => {
  const { items, subtotal, clearCart, restaurantSlug } = useCart();
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        ...
        >
          ...
          <p className="text-muted-foreground mt-2">
            Commande #CM-{Math.floor(Math.random() * 9000) + 1000}
          </p>
          <div className="mt-6 p-4 bg-secondary rounded-2xl text-sm text-muted-foreground">
            ...
            <p className="mt-2">
              {mode === "collect" ? "Récupérez votre commande sur place" : "Livraison en cours"}
            </p>
          </div>

          {/* Order Tracking */}
          <div className="mt-8 space-y-4 text-left">
            {["Confirmée", "En préparation", "Prête", mode === "collect" ? "À récupérer" : "En livraison"].map(
              (s, i) => (
          ...
          <Button onClick={() => navigate("/")} className="mt-8 w-full h-12 rounded-2xl" variant="outline">
            Retour à l'accueil
          </Button>
        </motion.div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {step === "mode" && (
            <motion.div key="mode" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-4">
              ...
              {(["collect", "delivery"] as Mode[]).map((m) => (
                <button
                  ...
                  className={`w-full p-4 rounded-2xl text-left transition-all border ${
                    mode === m ? "border-foreground bg-secondary" : "border-border hover:bg-secondary/50"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    {m === "collect" ? <MapPin className="h-5 w-5" /> : <Clock className="h-5 w-5" />}
                    <div>
                      <p className="font-medium text-foreground">
                        {m === "collect" ? "Click & Collect" : "Livraison"}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {m === "collect" ? "Récupérez sur place — Gratuit" : `À domicile — ${deliveryFee.toFixed(2)} €`}
                      </p>
                    </div>
                  </div>
                </button>
              ...
              <Button onClick={() => setStep("info")} className="w-full h-14 text-base font-semibold rounded-2xl" size="lg">
                Continuer
              </Button>
            </motion.div>
          )}
