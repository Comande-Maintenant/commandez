...
BEGIN
  -- This migration originally enriched a restaurant that existed only in the
  -- historical production database. Keep the enrichment for existing installs,
  -- but make clean database reconstruction deterministic.
  IF EXISTS (SELECT 1 FROM restaurants WHERE id = rid) THEN
  -- Bases (10)
